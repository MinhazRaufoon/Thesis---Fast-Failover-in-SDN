...
#####SAVE INITIAL GROUP TABLES IN FILES########
print "Writing the group entries to files..."
for switch in switches:
	result = switch.cmd('ovs-ofctl -O Openflow13 dump-groups',switch)
	open('InitialGroupTable_%s.txt'%switch,'w').write(result)
...
#####SAVE MODIFIED GROUP TABLES IN FILES########
print "Writing the group entries to files..."
for switch in switches:
	result = switch.cmd('ovs-ofctl -O Openflow13 dump-groups',switch)
	open('ModifiedGroupTable_%s.txt'%switch,'w').write(result)
